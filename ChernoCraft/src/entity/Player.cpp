...
void Player::update()
{
	...
	if (m_Window.IsMouseGrabbed())
	{
		...
		mouse.x = 960.0f / 2.0f - mouse.x;
		mouse.y = 540.0f / 2.0f - mouse.y;
		rotation.x -= mouse.y * mouse_sensitivity;
		rotation.y -= mouse.x * mouse_sensitivity;
		m_Window.SetMousePosition(vec2(960.0f / 2.0f, 540.0f / 2.0f));
	}
	...
	if (m_Window.IsMouseClicked(SP_MOUSE_LEFT))
	{
		if (!m_Window.IsMouseGrabbed())
		{
	...
	else if (m_Window.IsMousePressed(SP_MOUSE_RIGHT))
	{
		// Place block!
...
void Player::render()
{
	...
	shader->SetUniform("vw_matrix", vw_matrix);
	shader->SetUniform("player_pos", vec3(-position.x, -position.y, -position.z));

	if (light)
		shader->SetUniform("light_on", 1.0f);
	else
		shader->SetUniform("light_on", 0.0f);
