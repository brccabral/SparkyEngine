	...
	namespace graphics
	{
		namespace MeshFactory
		{
			...
			uint CreateQuad(float x, float y, float width, float height)
			{
				...
				VertexData data[4];
				data[0].vertex = maths::vec3(x, y, 0.0f);
				data[0].uv = maths::vec2(0.0f, 1.0f);

				data[1].vertex = maths::vec3(x, y + height, 0.0f);
				data[1].uv = maths::vec2(0.0f, 0.0f);

				data[2].vertex = maths::vec3(x + width, y + height, 0.0f);
				data[2].uv = maths::vec2(1.0f, 0.0f);

				data[3].vertex = maths::vec3(x + width, y, 0.0f);
				data[3].uv = maths::vec2(1.0f, 1.0f);
