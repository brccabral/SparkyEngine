...
namespace SparkyCLI {
	...
	{
		m_Instance->SetUniform(string_to_char_array(name), value);
	}

	void Shader::SetUniform1fv(System::String^ name, array<float>^ value, int count)
	{
		pin_ptr<float> p = &value[0];
		m_Instance->SetUniform(string_to_char_array(name), p, count);
	}
	...
	{
		m_Instance->SetUniform(string_to_char_array(name), *vector->GetHandle());
	}
	...
	{
		m_Instance->SetUniform(string_to_char_array(name), *matrix->GetHandle());
	}
